<?php
// Configura la zona horaria a México DF
date_default_timezone_set('America/Mexico_City');

// Obtiene la fecha y hora actuales en formato 'Y-m-d H:i:s'
...
$sqlVenta = "INSERT INTO Pedidos (FechaPedido, ClienteID, Surtido) VALUES (?, ?, ?)";
$stmtVenta = $con->prepare($sqlVenta);
$stmtVenta->bindParam(1, $fechaHoraActual); // Reemplaza con el ID del usuario
$stmtVenta->bindParam(2, $id_cliente); // Reemplaza con el ID del cliente
$stmtVenta->bindParam(3, $surtido); // Reemplaza con el total de la venta
$stmtVenta->execute();
...
for ($i = 0; $i < count($id_productos); $i++) {
    ...
    $sqlExistencia = "SELECT Stock FROM Productos WHERE ProductoID = ?";
    ...
    $sqlUpdateExistencia = "UPDATE Productos SET Stock = ? WHERE ProductoID = ?";
    ...
    $sqlDetalleVenta = "INSERT INTO DetallesPedidos (PedidoID, ProductoID, Cantidad, PrecioUnitario, Total) VALUES (?, ?, ?, ?, ?)";
    ...
    $stmtDetalleVenta->bindParam(5, $total);
...
$pdfFilePath = '/Users/carloseduardoguedea/Desktop/Macadamia/tickets/' . $UrlActual;
